...
ZEND_BEGIN_ARG_INFO_EX(arginfo_phalcon_security_derivekey, 0, 0, 2)
	...
	ZEND_ARG_TYPE_INFO(0, hash, IS_STRING, 1)
	ZEND_ARG_TYPE_INFO(0, iterations, IS_LONG, 1)
	ZEND_ARG_TYPE_INFO(0, size, IS_LONG, 1)
ZEND_END_ARG_INFO()
/*
ZEND_BEGIN_ARG_INFO_EX(arginfo_phalcon_security_pbkdf2, 0, 0, 2)
	ZEND_ARG_TYPE_INFO(0, password, IS_STRING, 0)
	ZEND_ARG_TYPE_INFO(0, salt, IS_STRING, 0)
	ZEND_ARG_TYPE_INFO(0, hash, IS_STRING, 1)
	ZEND_ARG_TYPE_INFO(0, iterations, IS_LONG, 1)
...
PHP_METHOD(Phalcon_Security, setRandomBytes){
	...
	phalcon_update_property(getThis(), SL("_numberBytes"), random_bytes);
}
...
PHP_METHOD(Phalcon_Security, setWorkFactor){
	...
	phalcon_update_property(getThis(), SL("_workFactor"), work_factor);
}
...
PHP_METHOD(Phalcon_Security, getSaltBytes)
{
	...
	if (number_bytes || Z_TYPE_P(number_bytes) != IS_NULL) {
	...
	if (phalcon_function_exists_ex(SL("openssl_random_pseudo_bytes")) == FAILURE) {
		...
		result = estrndup(Z_STRVAL(tmp), Z_STRLEN(tmp));
		zval_dtor(&tmp);
	}
...
PHP_METHOD(Phalcon_Security, hash)
{
	zval *password, *_work_factor = NULL, work_factor = {}, n_bytes = {}, salt_bytes = {}, default_hash = {}, z_salt = {};
	char variant, *salt;
	int salt_len, i_factor, i_hash;

	phalcon_fetch_params(0, 1, 1, &password, &_work_factor);

	if (_work_factor && Z_TYPE_P(_work_factor) != IS_NULL) {
		i_factor = Z_LVAL_P(_work_factor);
	} else {
		phalcon_read_property(&work_factor, getThis(), SL("_workFactor"), PH_NOISY|PH_READONLY);
		i_factor = Z_LVAL(work_factor);
	}

	phalcon_read_property(&default_hash, getThis(), SL("_defaultHash"), PH_READONLY);
	i_hash = Z_LVAL(default_hash);
...
PHP_METHOD(Phalcon_Security, checkHash){
	...
	if (max_pass_length && Z_TYPE_P(max_pass_length) != IS_NULL) {
...
PHP_METHOD(Phalcon_Security, getTokenKey){
	...
	if (phalcon_function_exists_ex(SL("openssl_random_pseudo_bytes")) == FAILURE) {
		PHALCON_MM_THROW_EXCEPTION_STR(phalcon_security_exception_ce, "Openssl extension must be loaded");
		return;
	}
	...
	if (!_number_bytes) {
		...
		ZVAL_COPY_VALUE(&number_bytes, _number_bytes);
	}
	...
	PHALCON_MM_ZVAL_STRING(&key, "$PHALCON/CSRF/KEY$");
	...
	PHALCON_MM_CALL_FUNCTION(&random_bytes, "openssl_random_pseudo_bytes", &number_bytes);
	PHALCON_MM_ADD_ENTRY(&random_bytes);
	phalcon_base64_encode(&base64bytes, &random_bytes);
	PHALCON_MM_ADD_ENTRY(&base64bytes);
	phalcon_filter_alphanum(&safe_bytes, &base64bytes);
	PHALCON_MM_ADD_ENTRY(&safe_bytes);

	PHALCON_MM_CALL_METHOD(NULL, &session, "set", &key, &safe_bytes);

	RETURN_MM_CTOR(&safe_bytes);
}
...
PHP_METHOD(Phalcon_Security, getToken){
	...
	PHALCON_MM_ZVAL_STRING(&key, "$PHALCON/CSRF$");
	...
	PHALCON_MM_ADD_ENTRY(&token);

	PHALCON_MM_CALL_METHOD(NULL, &session, "set", &key, &token);

	RETURN_MM_CTOR(&token);
}
...
PHP_METHOD(Phalcon_Security, checkToken){
	...
	phalcon_fetch_params(1, 0, 3, &name, &_token_key, &token_value);
	...
	if (!_token_key || Z_TYPE_P(_token_key) == IS_NULL) {
		...
		PHALCON_MM_CALL_METHOD(&token_key, &session, "get", &key);
		PHALCON_MM_ADD_ENTRY(&token_key);
	} else {
	...
	if (Z_TYPE_P(token_value) == IS_NULL) {
		...
		PHALCON_MM_CALL_METHOD(&request, getThis(), "getresolveservice", &service);
		PHALCON_MM_ADD_ENTRY(&request);
		PHALCON_MM_VERIFY_INTERFACE(&request, phalcon_http_requestinterface_ce);

		/**
		 * We always check if the value is correct in post
		 */
		PHALCON_MM_CALL_METHOD(&token, &request, "getpost", &token_key);
		PHALCON_MM_ADD_ENTRY(&token);
	} else {
		ZVAL_COPY_VALUE(&token, token_value);
	}
	...
	PHALCON_MM_CALL_METHOD(&session_token, &session, "get", &key);
	PHALCON_MM_ADD_ENTRY(&session_token);
...
PHP_METHOD(Phalcon_Security, getSessionToken){
	...
	PHALCON_MM_RETURN_CALL_METHOD(&session, "get", &key);
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Security, destroyToken){
	...
	if (PHALCON_IS_EMPTY(token_key)) {
		...
		PHALCON_MM_ZVAL_STRING(&key, Z_STRVAL_P(token_key));
	}
...
PHP_METHOD(Phalcon_Security, pbkdf2)
{
	...
	for (i = 1; i <= block_count; ++i) {
		...
		concat_function(&result, &result, &K2);
		zval_ptr_dtor(&K2);
		zval_ptr_dtor(&K1);
	}
	efree(s);
...
PHP_METHOD(Phalcon_Security, setDefaultHash)
{
	...
	phalcon_update_property(getThis(), SL("_defaultHash"), default_hash);
}
